...
public interface TestManagementProvider {
  ...
  /**
   * One page of the provider's tests, filtered by a free-text {@code query}, or a native query when
   * {@code advanced} is true. Paged rather than a bare list: the answer to "the test is not in the
   * first 20" has to be a next page, and the total is what tells the caller whether one exists.
   */
